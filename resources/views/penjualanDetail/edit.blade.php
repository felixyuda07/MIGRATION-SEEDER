...
@section('content')
    <div class="card card-outline card-primary">
        ...
        <div class="card-body">
            ...
            @else
                <form method="POST" action="{{ url('/penjualanDetail/' . $penjual->penjualan_detail_id) }}" class="form-horizontal">
                    @csrf
        ...
        yang butuh method PUT -->
                        ...
                        <div class="col-11">
                            <select class="form-control" id="Barang_id" name="barang_id" required>
                                ...
                                @foreach ($barang as $item)
                                    <option value="{{ $item->barang_id }}" @if ($item->barang_id == $penjual->barang_id) selected @endif>
                                        {{ $item->harga_barang_barang }}</option>
                                @endforeach
                            </select>
                            @error('barang_id')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-11">
                            <input type="date" class="form-control" id="penjualan_id" name="penjualan_id" value="{{ old('penjualan_id', $penjual->penjualan_id) }}" required>
                            @error('penjualan_id')
                                <small class="form-text text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-11">
                            <input type="integer" class="form-control" id="jumlah_barang" name="jumlah_barang"
                                value="{{ old('jumlah_barang', $penjual->jumlah_barang) }}" required>
                            @error('jumlah_barang')
                                <small class="form-text text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-11">
                            <input type="number" class="form-control" id="harga_barang" name="harga_barang"
                                value="{{ old('harga_barang', $penjual->harga_barang) }}" required>
                            @error('harga_barang')
                                <small class="form-text text-danger">{{ $message }}</small>
                            @enderror
                        </div>
